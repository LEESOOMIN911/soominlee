...
---------------------------------------------------------------------------------------------- 
CREATE TABLE MCARE_USER_STATE 
(
	P_ID 	 VARCHAR2(255), 
	STATE_CD VARCHAR2(8), 
	STATE_DT DATE,
	CONSTRAINT FK_P_ID_STATE FOREIGN KEY (P_ID) REFERENCES MCARE_USER (P_ID) 
); 

COMMENT ON COLUMN MCARE_USER_STATE.P_ID IS '환자번호'; 
COMMENT ON COLUMN MCARE_USER_STATE.STATE_CD IS '상태 (도우미가 참고용)'; 
COMMENT ON COLUMN MCARE_USER_STATE.STATE_DT IS '상태전이일시'; 
